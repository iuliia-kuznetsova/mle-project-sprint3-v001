# ----- INSTRUCTIONS -----#
# ------ STAGE 3: TESTING AND MONITORING MICROSERVICE -----#
'''
  Testing the FastAPI microservice, monitoring using Prometeus and building dashbord using Grafana.
'''
...
- dockerfile `Dockerfile_stage_3` in `services/`
- api script `main_stage_3.py` in `services/ml_service`
- testing script `test_api.py` in `services/ml_service`

# Check wheither ports are aleady occupied 
# (terminal should return nothing, if something exists it's needed to change ports at .env, Dockerfile_stage_3_4, prometheus.yml)
```bash
sudo lsof -i :8080
sudo lsof -i :9091
sudo lsof -i :3001
...
# Run test script
``` bash
python3 services/ml_service/test_api.py
``` 
